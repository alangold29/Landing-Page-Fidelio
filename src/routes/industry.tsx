...
export function IndustryLandingPage({ slug }: { slug: IndustrySlug }) {
  ...
  return <main className="min-h-screen bg-background px-5 pb-20 pt-28 sm:px-8 md:px-12 md:pb-32 md:pt-36">
    ...
    <section id="demo" className="mt-28 flex flex-col items-start justify-between gap-8 border-t border-border pt-10 md:flex-row md:items-end"><div><p className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">Taply para {profile.label.toLowerCase()}</p><h2 className="mt-5 text-4xl font-medium tracking-[-.035em] md:text-5xl">Diseñemos el programa<br/>para tu negocio.</h2></div><a href="https://cal.com/alan-goldstein-z1hmxl/demo-fidelio" className="inline-flex items-center gap-3 rounded-full bg-foreground py-2 pl-6 pr-2 text-sm font-medium text-white transition hover:bg-foreground/85">Reserva una demo<span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#9ae8af] text-foreground"><ArrowUpRight className="h-4 w-4"/></span></a></section>
  </main>
}
