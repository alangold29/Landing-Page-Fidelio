...
type IndustryProfile = {
  ...
  messages: string[]
  scene: { label: string; detail: string; tone?: 'light' | 'dark'; src?: string; alt?: string }
  supportImage?: { src: string; alt: string; caption: string }
}

export const industryProfiles: Record<IndustrySlug, IndustryProfile> = {
  cafeterias: {
    label: 'Cafeterías', eyebrow: 'Fidelio para cafeterías', title: <>Haz que el café de hoy<br/>traiga la <span>próxima visita.</span></>,
    ...
    scene: { label: 'Café de rutina', detail: 'Clientes habituales conversando junto a un QR de fidelización en una cafetería local', src: '/images/fidelio-cafeterias-amigos.webp', alt: 'Dos clientes habituales conversando en una cafetería local junto a un QR de fidelización' },
    supportImage: { src: '/images/fidelio-cafeterias-barra.webp', alt: 'Barista entregando un café a un cliente habitual junto a un QR de fidelización', caption: 'Una visita cotidiana también puede construir la siguiente.' },
  },
  bares: {
    label: 'Bares', eyebrow: 'Fidelio para bares', title: <>Haz que una noche buena<br/>se convierta en <span>la próxima salida.</span></>,
    ...
    scene: { label: 'Noche de bar', detail: 'Amigos en un bar local con un QR de fidelización sobre la mesa', tone: 'dark', src: '/images/fidelio-bares-amigos.webp', alt: 'Grupo de amigos en un bar local junto a un código QR de fidelización' },
    supportImage: { src: '/images/fidelio-bares-barra.webp', alt: 'Bartender atendiendo a un cliente habitual en un bar local', caption: 'Una visita cotidiana al bar se puede convertir en la siguiente.' },
  },
  restaurantes: {
    label: 'Restaurantes', eyebrow: 'Fidelio para restaurantes', title: <>Convierte una buena mesa<br/>en una <span>próxima reserva.</span></>,
    ...
    scene: { label: 'La mesa que se repite', detail: 'Amigos compartiendo una cena junto a un QR de fidelización', src: '/images/fidelio-restaurantes-amigos.webp', alt: 'Grupo de amigos compartiendo una cena en un restaurante local junto a un QR de fidelización' },
    supportImage: { src: '/images/fidelio-restaurantes-barra.webp', alt: 'Mesero atendiendo a un cliente habitual en un restaurante local', caption: 'La relación continúa después de una buena mesa.' },
  },
  comercios: {
    label: 'Comercios', eyebrow: 'Fidelio para comercios', title: <>Haz que una compra<br/>sea el inicio de <span>la siguiente.</span></>,
    ...
    scene: { label: 'Compra de confianza', detail: 'Clienta atendida por la dueña de un comercio local junto a un QR de fidelización', src: '/images/fidelio-comercios-amigos.webp', alt: 'Clienta y dueña de un comercio local durante una compra recurrente' },
    supportImage: { src: '/images/fidelio-comercios-barra.webp', alt: 'Dueña de un comercio entregando una compra a una clienta habitual', caption: 'Una compra puede abrir la puerta a la siguiente.' },
  },
  panaderias: {
    label: 'Panaderías', eyebrow: 'Fidelio para panaderías', title: <>Convierte compras diarias<br/>en un <span>hábito que vuelve.</span></>,
    ...
    scene: { label: 'La compra de cada día', detail: 'Clienta en una panadería local durante una compra habitual', src: '/images/fidelio-panaderias-amigos.webp', alt: 'Clienta y panadera intercambiando pan fresco en una panadería local' },
    supportImage: { src: '/images/fidelio-panaderias-barra.webp', alt: 'Panadera entregando pan fresco a un cliente habitual junto a un QR de fidelización', caption: 'La rutina de compra también merece volver.' },
  },
  eventos: {
    label: 'Eventos', eyebrow: 'Fidelio para eventos', title: <>Tu evento sigue presente<br/>desde el <span>primer acceso.</span></>,
    ...
    scene: { label: 'La experiencia empieza al llegar', detail: 'Amigos entrando a un venue local junto a un acceso digital', tone: 'dark', src: '/images/fidelio-eventos-amigos.webp', alt: 'Dos asistentes entrando a un evento local con acceso digital' },
    supportImage: { src: '/images/fidelio-eventos-barra.webp', alt: 'Host dando la bienvenida a una asistente con acceso digital', caption: 'El acceso también puede mantener la relación activa.' },
  },
}
...
export function IndustryLandingPage({ slug }: { slug: IndustrySlug }) {
  ...
  useEffect(() => {
    document.title = `${profile.label} | Fidelio`
  ...
  return <main className="min-h-screen bg-background px-5 pb-20 pt-28 sm:px-8 md:px-12 md:pb-32 md:pt-36">
    ...
    <section className="mt-28 grid grid-cols-12 items-center gap-12 bg-foreground px-7 py-12 text-white md:px-12 md:py-16"><div className="col-span-12 md:col-span-5"><Bell className="h-6 w-6 text-[#9ae8af]" strokeWidth={1.5}/><p className="mt-12 text-[11px] uppercase tracking-[0.2em] text-white/45">Comunicación que vuelve a abrir la puerta</p><h2 className="mt-6 text-4xl font-medium leading-tight tracking-[-.03em]">Un mensaje útil, en el momento adecuado.</h2></div>{profile.supportImage && <figure className="col-span-12 overflow-hidden rounded-[18px] border border-white/10 md:col-span-3 md:col-start-7"><img className="aspect-[4/5] h-full w-full object-cover" src={profile.supportImage.src} alt={profile.supportImage.alt}/><figcaption className="sr-only">{profile.supportImage.caption}</figcaption></figure>}<div className={`col-span-12 ${profile.supportImage ? 'md:col-span-3 md:col-start-10' : 'md:col-span-5 md:col-start-8'}`}><p className="text-base leading-relaxed text-white/60">Las notificaciones no tienen que ser genéricas. Cada una puede tener un motivo real para que la siguiente visita ocurra.</p><div className="mt-8 space-y-3 border-t border-white/15 pt-6">{profile.messages.map(message => <p key={message} className="flex items-start gap-3 text-sm text-white/75"><Zap className="mt-0.5 h-4 w-4 shrink-0 text-[#9ae8af]"/>{message}</p>)}</div></div></section>

    <section className="mt-28 grid grid-cols-12 items-end gap-12"><div className="col-span-12 md:col-span-7"><ScenePlaceholder {...profile.scene}/></div><div className="col-span-12 md:col-span-4 md:col-start-9"><p className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">Hecho para tu operación</p><h2 className="mt-7 text-4xl font-medium leading-tight tracking-[-.03em]">Más frecuencia.<br/><span className="text-muted-foreground">Más retorno.</span></h2><p className="mt-6 text-base leading-relaxed text-muted-foreground">Empieza con un QR en tu punto de venta, una tarjeta en Wallet y una recompensa que tenga sentido para tu clientela.</p><a href="https://cal.com/alan-goldstein-z1hmxl/demo-fidelio" className="mt-8 inline-flex items-center gap-2 text-sm font-medium text-foreground">Reserva una demo <ArrowDown className="h-4 w-4"/></a></div></section>

    <section id="demo" className="mt-28 flex flex-col items-start justify-between gap-8 border-t border-border pt-10 md:flex-row md:items-end"><div><p className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">Fidelio para {profile.label.toLowerCase()}</p><h2 className="mt-5 text-4xl font-medium tracking-[-.035em] md:text-5xl">Diseñemos el programa<br/>para tu negocio.</h2></div><a href="https://cal.com/alan-goldstein-z1hmxl/demo-fidelio" className="inline-flex items-center gap-3 rounded-full bg-foreground py-2 pl-6 pr-2 text-sm font-medium text-white transition hover:bg-foreground/85">Reserva una demo<span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#9ae8af] text-foreground"><ArrowUpRight className="h-4 w-4"/></span></a></section>
  </main>
}
