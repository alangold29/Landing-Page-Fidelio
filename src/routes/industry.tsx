...
export function IndustryLandingPage({ slug }: { slug: IndustrySlug }) {
  ...
  return <main className="min-h-screen bg-background px-5 pb-20 pt-28 sm:px-8 md:px-12 md:pb-32 md:pt-36">
    ...
    <section className="mt-28 grid grid-cols-12 items-center gap-12 bg-foreground px-7 py-12 text-white md:px-12 md:py-16"><div className="col-span-12 md:col-span-5"><Bell className="h-6 w-6 text-[#9ae8af]" strokeWidth={1.5}/><p className="mt-12 text-[11px] uppercase tracking-[0.2em] text-white/45">Comunicación que vuelve a abrir la puerta</p><h2 className="mt-6 text-4xl font-medium leading-tight tracking-[-.03em]">Un mensaje útil, en el momento adecuado.</h2></div>{slug === 'bares' && <figure className="col-span-12 overflow-hidden rounded-[18px] border border-white/10 md:col-span-3 md:col-start-7"><img className="aspect-[4/5] h-full w-full object-cover" src="/images/fidelio-bares-barra.webp" alt="Bartender atendiendo a un cliente habitual en un bar local"/><figcaption className="sr-only">Una visita cotidiana al bar se puede convertir en la siguiente.</figcaption></figure>}<div className={`col-span-12 ${slug === 'bares' ? 'md:col-span-3 md:col-start-10' : 'md:col-span-5 md:col-start-8'}`}><p className="text-base leading-relaxed text-white/60">Las notificaciones no tienen que ser genéricas. Cada una puede tener un motivo real para que la siguiente visita ocurra.</p><div className="mt-8 space-y-3 border-t border-white/15 pt-6">{profile.messages.map(message => <p key={message} className="flex items-start gap-3 text-sm text-white/75"><Zap className="mt-0.5 h-4 w-4 shrink-0 text-[#9ae8af]"/>{message}</p>)}</div></div></section>
