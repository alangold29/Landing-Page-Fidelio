...
export function Header() {
  const getRoute = () => window.location.pathname === '/' ? (window.location.hash || '/') : window.location.pathname
  ...
  return <header className="fixed left-0 right-0 top-4 z-50 flex items-center justify-between gap-3 px-5 sm:px-8 md:top-6 md:px-12">
    <a href="/" aria-label="Taply, inicio" className={`shrink-0 font-display text-2xl font-medium tracking-[-.04em] ${logoTheme}`}>Taply<span className="text-[#72c888]">.</span></a>
    <div className="flex items-center gap-3">
      <nav aria-label="Navegación principal" className={`hidden items-center gap-0.5 rounded-full p-1.5 backdrop-blur-md sm:flex ${navTheme}`}>
        <a href="/" aria-current={active('/') ? 'page' : undefined} className={linkClass(active('/'))}>Home</a>
    ...
    {menuOpen && <nav aria-label="Navegación móvil" className={`absolute right-5 top-full mt-3 flex min-w-[250px] flex-col gap-1 rounded-2xl p-2 shadow-[0_16px_42px_rgba(20,25,20,.16)] backdrop-blur-md sm:hidden ${navTheme}`}><a href="/" className={`rounded-xl px-4 py-3 text-sm font-medium transition ${active('/') ? (pastHero ? 'bg-foreground text-white' : 'bg-white/12') : 'opacity-75 hover:opacity-100'}`}>Home</a><p className="px-4 pt-3 text-[11px] uppercase tracking-[0.16em] opacity-45">Producto</p>{productLinks.map(([href, label]) => <a key={href} href={href} className={`rounded-xl px-4 py-2.5 text-sm transition ${active(href) ? (pastHero ? 'bg-foreground text-white' : 'bg-white/12') : 'opacity-75 hover:opacity-100'}`}>{label}</a>)}<div ref={mobileIndustryMenuRef} className="mt-1"><div className={`flex items-center justify-between rounded-xl text-sm font-medium transition ${active('/industrias') || industryOpen ? (pastHero ? 'bg-foreground text-white' : 'bg-white/12') : 'opacity-75 hover:opacity-100'}`}><a href="/industrias" className="flex-1 px-4 py-3">Industrias</a><button type="button" aria-label="Abrir industrias" aria-expanded={industryOpen} onClick={() => setIndustryOpen(open => !open)} className="mr-2 rounded-lg p-2"><ChevronDown className={`h-4 w-4 transition-transform ${industryOpen ? 'rotate-180' : ''}`}/></button></div>{industryOpen && <div className="mt-1 grid grid-cols-2 gap-1 px-1">{industryLinks.map(([href, label]) => <a key={href} href={href} className={`rounded-lg px-3 py-2.5 text-sm transition ${active(href) ? (pastHero ? 'bg-foreground text-white' : 'bg-white/12') : 'opacity-70 hover:opacity-100'}`}>{label}</a>)}</div>}</div><a href="https://cal.com/alan-goldstein-z1hmxl/demo-fidelio" className="mt-1 flex items-center justify-between rounded-xl bg-[#9ae8af] px-4 py-3 text-sm font-medium text-[#122016]">Reserva una demo <ArrowUpRight className="h-4 w-4" /></a></nav>}
  </header>
}
