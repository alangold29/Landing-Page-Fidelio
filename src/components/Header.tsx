...
export function Header() {
  ...
  const spanishHref = getRoute()
  const portugueseHref = localizedPath(getRoute(), 'pt')
  ...
  return <header className="fixed left-0 right-0 top-4 z-50 flex items-center justify-between gap-3 px-5 sm:px-8 md:top-6 md:px-12">
    ...
    <div className="flex items-center gap-3">
      ...
      <div aria-label="Selector de idioma" className={`hidden items-center rounded-full p-1 text-xs font-medium sm:flex ${pastHero ? 'bg-foreground/5 text-foreground' : 'bg-white/10 text-white'}`}>
        <a data-locale-switch href={spanishHref} aria-current={locale === 'es' ? 'true' : undefined} className={`rounded-full px-3 py-1.5 transition ${locale === 'es' ? (pastHero ? 'bg-foreground text-white' : 'bg-white text-foreground') : 'opacity-60 hover:opacity-100'}`}>Español</a>
        <a data-locale-switch href={portugueseHref} aria-current={locale === 'pt' ? 'true' : undefined} className={`rounded-full px-3 py-1.5 transition ${locale === 'pt' ? (pastHero ? 'bg-foreground text-white' : 'bg-white text-foreground') : 'opacity-60 hover:opacity-100'}`}>Português</a>
      </div>
