...
export function ScenePlaceholder({ label, detail, tone = 'light', src, alt, className = '' }: { label: string; detail: string; tone?: 'light' | 'dark'; src?: string; alt?: string; className?: string }) {
  if (src) return <figure className={`scene-placeholder scene-placeholder-photo ${className}`}><img src={src} alt={alt ?? detail}/><figcaption className="sr-only">{label}. {detail}</figcaption></figure>
