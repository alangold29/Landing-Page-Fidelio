...
export function ScenePlaceholder({ label, detail, tone = 'light', src, alt }: { label: string; detail: string; tone?: 'light' | 'dark'; src?: string; alt?: string }) {
  if (src) return <figure className="scene-placeholder scene-placeholder-photo"><img src={src} alt={alt ?? detail}/><figcaption className="sr-only">{label}. {detail}</figcaption></figure>
